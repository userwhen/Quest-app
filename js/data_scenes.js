/* js/data_scenes.js - V22.0 Massive Content Expansion */
...
window.StoryData = {
    // ==========================================
    // 1. 擴充詞庫 (Rich WordBank)
    // ==========================================
    commonWordBank: {
        Weather: [ 
            {text:"晴朗的"}, {text:"陰雨綿綿的"}, {text:"起大霧的"}, {text:"狂風大作的"}, {text:"陽光普照的"}, {text:"飄著細雪的"}
        ],
        Time: [ 
            {text:"清晨"}, {text:"正午"}, {text:"黃昏"}, {text:"深夜"}, {text:"黎明前"}, {text:"午後"}
        ],
        Atmosphere: [
            {text:"空氣中瀰漫著緊張的氣息"}, {text:"周圍安靜得令人發毛"}, {text:"遠處傳來熱鬧的喧囂聲"}, {text:"這裡充滿了腐敗的氣味"}
        ],
        Sound: [
            {text:"一陣急促的腳步聲"}, {text:"金屬碰撞的聲音"}, {text:"低沉的啜泣聲"}, {text:"詭異的笑聲"}, {text:"草叢的沙沙聲"}
        ],
        Adjective: [ 
            {text:"神色慌張的"}, {text:"喝得爛醉的"}, {text:"受傷的"}, {text:"看起來很有錢的"}, {text:"面露兇光的"}, 
            {text:"衣衫襤褸的"}, {text:"全副武裝的"}, {text:"神秘兮兮的"}
        ]
    },
    ...
    wordBanks: {
        ...
        adventurer: {
            Person: [ 
                ...
                {text:"老練的獵人", weight:5}, {text:"吟遊詩人", weight:5},
                {text:"哥布林斥候", weight:5}, {text:"強盜", weight:8}, {text:"逃兵", weight:3},
                {text:"賞金獵人", weight:5}, {text:"神秘的法師", weight:2}
            ],
            Place: [ 
                {text:"城鎮廣場"}, {text:"熱鬧的酒館"}, {text:"冒險者公會門口"}, {text:"擁擠的市場"},
                {text:"陰暗的小巷"}, {text:"廢棄礦坑入口"}, {text:"迷霧森林邊緣"}, {text:"泥濘的沼澤地"},
                {text:"古老的遺跡"}, {text:"國境檢查哨"}
            ],
            Item: [ 
        ...
        harem: {
            Person: [ 
                ...
                {text:"新進的秀女", weight:5}, {text:"掌事姑姑", weight:5}
            ],
            Place: [ 
                {text:"御花園的涼亭"}, {text:"無人的長廊"}, {text:"冷宮門口"}, {text:"御膳房後門"}, {text:"荷花池畔"},
                {text:"藏書閣"}, {text:"內務府"}, {text:"皇后的寢宮外"}
            ],
            Item: [ 
    ...
    // ==========================================
    // 2. 模組化劇情語法 (Massive Grammar)
    // 邏輯：隨機抽取 1個 Setup + 1個 Event + 1個 Conflict
    // ==========================================
    plotGrammar: {
        setups: [
            // 環境類
        ...
        events: [
            ...
            { text: "一位{Person}正在大聲叫賣著什麼。" },
            { text: "一位{Person}鬼鬼祟祟地在尋找東西。" }
        ],
        conflicts: [
            // A. 求助/互動
            { text: "他看起來遇到了麻煩，詢問你是否能提供幫助。", options: [
                { label: "提供幫助 (消耗精力)", rewards: [{type:'energy', val:-5}, {type:'exp', val:20}], next: 'event_check_pass' },
                { label: "無視離開", end: true },
                { label: "趁火打劫", check: { stat: 'str', val: 10 }, pass: 'event_rob_win', fail: 'event_fight_lose' }
            ]},
            // B. 敵意/衝突
            ...
            { text: "他指著那個物品大喊：「那是我的！」", options: [
                ...
                { label: "據為己有", check: { stat: 'str', val: 10 }, pass: 'event_item_get', fail: 'event_fight_lose' }
            ]},
            // F. 發現傷患
            { text: "他看起來受了重傷，奄奄一息。", options: [
                { label: "使用傷藥", req: { tag: 'potion' }, rewards: [{type:'exp', val:50}, {type:'gold', val:20}], next: 'event_check_pass' }, // 需實作Tag
    ...
    scenes: {
        ...
        'event_check_fail': { text: "事情搞砸了...你只能尷尬地離開。", rewards: [{type:'energy', val:-5}], end: true },
        ...
        // --- 長篇副本 (Dungeons) ---
        'dwarf_mine': {
            text: "你發現了一個古老的矮人礦坑入口，裡面傳來金屬敲擊聲。", bg: '⚒️',
            options: [{ label: "進入探索", next: 'mine_explore' }, { label: "離開", end: true }]
        },
        'mine_explore': {
            text: "坑道深處有一個巨大的秘銀齒輪鎖。",
            options: [
                { label: "暴力拆解 (力量)", check: { stat: 'str', val: 14 }, pass: 'mine_win', fail: 'mine_lose' },
                { label: "解鎖 (智力)", check: { stat: 'int', val: 14 }, pass: 'mine_win', fail: 'mine_trap' }
            ]
        },
        ...
        'tower_start': {
            text: "荒原上矗立著一座紫色雷電環繞的古塔。傳說塔頂有許願石。", bg: '🗼',
            options: [
                { label: "正面破門", check: { stat: 'str', val: 10 }, pass: 'tower_enter', fail: 'tower_fail' },
                { label: "離開", end: true }
            ]
        },
        ...
        // --- 主線 (Main Story) ---
        'main_adv_0': {
            index: 0, title: "序章：冒險起點", bg: '📜',
            text: "你在公會註冊成為冒險者。會長看著你：「新人，先去森林邊緣調查一下吧。」",
            options: [{ label: "接受委託", end: true }]
        },
        'main_adv_1': {
            index: 1, reqLv: 2, title: "第一章：森林異變", bg: '🌲', 
